...
# 获取最新的发行版标签
latest_release=$(curl --silent "https://api.github.com/repos/eun1e/sublinkE/releases/latest" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
...
# 下载menu.sh并设置权限
curl -o /usr/bin/sublink -H "Cache-Control: no-cache" -H "Pragma: no-cache" https://raw.githubusercontent.com/eun1e/sublinkE/main/menu.sh
